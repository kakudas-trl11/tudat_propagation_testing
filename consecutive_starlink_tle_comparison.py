...
if __name__ == "__main__":
    ...
    STARLINK_ID = "STARLINK-3107" #32638
    ...
    for i in range(len(tles)-1):
        t0_line_1, t0_line_2 = tles[i]
        t1_line_1, t1_line_2 = tles[i+1]
        ...
        tle_epochs.append(datetime(year1, month1, day1, hour1, minute1, second1, tzinfo=timezone.utc))
        ...
        acceleration_settings_starlink = dict(
            ...
            Earth=[
                # propagation_setup.acceleration.point_mass_gravity(),
                propagation_setup.acceleration.spherical_harmonic_gravity(8, 8), # Changing to spherical harmonics made a big difference
            ]
        )
    ...
    plt.semilogy(tle_epochs, sgp4_pos_error, 'o-', label='SGP4 Position Error (km)')
    plt.semilogy(tle_epochs, tudat_pos_error, 's-', label='Tudat Position Error (km)')
    plt.xlabel('TLE Epoch (UTC)')
    plt.ylabel('Position Error (km)')
    plt.title(f'{STARLINK_ID} Consecutive Position Error Comparison: Tudat vs SGP4')
    plt.legend()
    plt.grid()
    plt.savefig(f'{STARLINK_ID}_consecutive_position_error_comparison.png', dpi=300)
    # plt.show()